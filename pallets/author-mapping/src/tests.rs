...
use crate::{
	mock::{
		last_event, AuthorMapping, Balances, Event as MetaEvent, ExtBuilder, Origin, Runtime,
		System, TestAuthor,
	},
	Error, Event,
};
...
fn eligible_account_can_register() {
	ExtBuilder::default().with_balances(vec![(2, 1000)]).build().execute_with(|| {
		assert_ok!(AuthorMapping::add_association(Origin::signed(2), TestAuthor::Bob.into()));
...
fn double_registration_costs_twice_as_much() {
	ExtBuilder::default().with_balances(vec![(2, 1000)]).build().execute_with(|| {
		...
		assert_ok!(AuthorMapping::add_association(Origin::signed(2), TestAuthor::Alice.into()));
